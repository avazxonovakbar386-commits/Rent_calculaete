...
export default function GoogleLoginButton({ text = 'Google bilan kirish' }: GoogleLoginButtonProps) {
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            if (error.code === 'auth/popup-closed-by-user') {
                toast.error('Google kirish oynasi bekor qilindi');
            } else if (error.code === 'auth/popup-blocked') {
                toast.error('Popup bloklangan. Brauzer sozlamalarini tekshiring');
            } else if (error.code === 'auth/network-request-failed') {
                toast.error("Internet aloqasida xatolik. Qayta urinib ko'ring");
            } else {
